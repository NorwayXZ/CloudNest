...
class ValidationTests(unittest.TestCase):
    def test_preflight_report_has_direct_capacity_planning_workflow(self):
        index_path = os.path.join(os.path.dirname(app.__file__), "static", "index.html")
        with open(index_path, encoding="utf-8") as handle:
            page = handle.read()
        for marker in (
            'data-preflight-report=',
            'id="preflightCapacityImage"',
            'id="preflightCapacityStrategy"',
            'function openPreflightReport(node)',
            "completedPreflight=cluster.tasks.find",
            "'/api/scheduler/plan'",
        ):
            self.assertIn(marker, page)

    def test_panel_version_and_remote_update_check(self):
        self.assertEqual(app.APP_VERSION, "2.7.0")
        self.assertLess(app.version_tuple("1.6.4"), app.version_tuple("1.7.0"))
        response = mock.MagicMock()
        response.read.return_value = b"2.7.1\n"
        response.__enter__.return_value = response
        with mock.patch("app.urlopen", return_value=response) as urlopen:
            self.assertEqual(app.fetch_latest_version(), "2.7.1")
        ...
        self.assertEqual(
            app.UPDATE_REPOSITORY_URL, "https://github.com/NorwayXZ/CloudNest",
        )
        ...
        with mock.patch("app.read_update_status", return_value={
            "status": "running", "target_version": "2.7.1",
        }):
            payload = app.panel_version_payload(refresh=False)
        self.assertEqual(payload["latest_version"], "2.7.1")
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                with mock.patch("app.subprocess.run", return_value=completed) as run:
                    status = app.start_panel_update("2.7.1")
                ...
                self.assertEqual(saved["target_version"], "2.7.1")
        finally:
    ...
    def test_admin_can_check_and_start_panel_update(self):
        ...
        try:
            ...
            with mock.patch.object(app.Handler, "log_message", return_value=None), \
                ...
                self.assertEqual(data["panel_version"], "2.7.0")
    ...
    def test_node_installer_requires_incus_server_package(self):
        ...
        for command in ("cloudnest-node-uninstall", "incus-cn-node-uninstall"):
            self.assertIn(f"/usr/local/sbin/{command}", source)
            self.assertIn(f"/usr/local/sbin/{command}", uninstaller)
        self.assertLess(
            source.index("/usr/local/sbin/cloudnest-node-uninstall"),
    ...
    def test_control_installer_includes_version_updater(self):
        ...
        for value in ("password.env", "password_config_rewrite=false", "chmod 0600"):
            self.assertIn(value, installer)
        for command in ("cloudnest-uninstall", "incus-cn-panel-uninstall"):
            self.assertIn(f"/usr/local/sbin/{command}", installer)
            self.assertIn(f"/usr/local/sbin/{command}", uninstaller)
        self.assertIn("incus-cn-panel-update", uninstaller)

    def test_bootstrap_scripts_use_cloudnest_repository_with_legacy_overrides(self):
        root = os.path.dirname(app.__file__)
        for filename in ("bootstrap.sh", "bootstrap-node.sh", "update.sh"):
            with self.subTest(filename=filename), open(
                os.path.join(root, filename), encoding="utf-8",
            ) as source_file:
                source = source_file.read()
            self.assertIn(
                "CLOUDNEST_REPOSITORY:-${INCUS_CN_REPOSITORY:-NorwayXZ/CloudNest}",
                source,
            )
            self.assertIn(
                "CLOUDNEST_BRANCH:-${INCUS_CN_BRANCH:-main}",
                source,
            )
    ...
    def test_node_ownership_is_private_and_admin_visible(self):
        old_values = (app.DATA_DIR, app.USERS_FILE, app.NODE_OWNERS_FILE, app.PANEL_USER)
        try:
            with tempfile.TemporaryDirectory() as directory:
                app.DATA_DIR = directory
                app.USERS_FILE = os.path.join(directory, "users.json")
                app.NODE_OWNERS_FILE = os.path.join(directory, "node-owners.json")
                app.PANEL_USER = "admin"
                app.create_user_account("customer-a", "123456")
                app.create_user_account("customer-b", "123456")
                app.set_node_owner("node-a", "customer-a")
                app.set_node_owner("node-b", "customer-b")
                owner_a = {"username": "customer-a", "role": "user"}
                owner_b = {"username": "customer-b", "role": "user"}
                admin = {"username": "admin", "role": "admin"}
                self.assertTrue(app.session_can_access_node(owner_a, "node-a"))
                self.assertFalse(app.session_can_access_node(owner_a, "node-b"))
                self.assertTrue(app.session_can_access_node(admin, "node-b"))
                self.assertTrue(app.session_can_manage_node(owner_a, "node-a"))
                self.assertFalse(app.session_can_manage_node(owner_a, "node-b"))
                with mock.patch("app.overview", return_value=([], [])):
                    users = {item["username"]: item for item in app.list_user_accounts()}
                self.assertEqual(users["customer-a"]["owned_nodes"], ["node-a"])
                self.assertEqual(users["customer-b"]["owned_nodes"], ["node-b"])
        finally:
            app.DATA_DIR, app.USERS_FILE, app.NODE_OWNERS_FILE, app.PANEL_USER = old_values
    ...
    def test_expiring_assignments_filter_user_overview_and_access(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                self.assertTrue(payload["public_images"])
        ...
        try:
            with tempfile.TemporaryDirectory() as directory:
                ...
                with mock.patch.object(app.Handler, "log_message", return_value=None), \
                    ...
                    self.assertEqual(request("GET", "/api/nodes/live")[0], 200)
