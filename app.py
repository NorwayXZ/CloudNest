...
def fetch_latest_version():
    request = Request(
        UPDATE_VERSION_URL,
        headers={"Accept": "text/plain", "User-Agent": f"CloudNest/{APP_VERSION}"},
    )
    try:
...
def test_telegram_notification():
    config = read_notification_config()
    now = datetime.now(timezone.utc).isoformat(timespec="seconds")
    send_telegram_message(config, f"[测试] CloudNest\nTelegram 异常通知连接正常。\n时间: {now}")
    return True
...
def write_caddy_routes(data):
    lines = ["# Generated by CloudNest. Do not edit manually.", ""]
...
class Handler(BaseHTTPRequestHandler):
    server_version = f"CloudNest/{APP_VERSION}"
...
def main():
    ...
    print(f"CloudNest 虚拟化集群面板正在监听 https://{HOST}:{PORT}", flush=True)
