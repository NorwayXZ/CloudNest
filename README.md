...
- 普通账户可以添加并管理自己的宿主机和实例；也可以查看管理员授予的有效实例、SSH 信息，并启动、停止或重启获授权实例
